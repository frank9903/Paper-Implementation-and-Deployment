"""
Some model graphs that are too long for notebook
...
class PyramidROIAlign(engine.Layer):
    ...
    def call(self, inputs):
        ...
        image_shape = data.parse_image_meta_graph(image_meta)['image_shape'][0]
...
class DetectionLayer(engine.Layer):
    ...
    def call(self, inputs):
        ...
        m = data.parse_image_meta_graph(image_meta)
...
def rpn_class_loss_graph(rpn_match, rpn_class_logits):
    ...
    indices = tf.where(backend.not_equal(rpn_match, 0))
    ...
    loss = backend.sparse_categorical_crossentropy(target=anchor_class,
                                             ...
                                             from_logits=True)
    loss = backend.switch(tf.size(loss) > 0, backend.mean(loss), tf.constant(0.0))
    return loss
...
def rpn_bbox_loss_graph(config, target_bbox, rpn_match, rpn_bbox):
    ...
    indices = tf.where(backend.equal(rpn_match, 1))
...
def mrcnn_bbox_loss_graph(target_bbox, target_class_ids, pred_bbox):
    ...
    # Smooth-L1 Loss
    loss = backend.switch(tf.size(target_bbox) > 0,
...
def mrcnn_mask_loss_graph(target_masks, target_class_ids, pred_masks):
    ...
    loss = backend.switch(tf.size(y_true) > 0,
